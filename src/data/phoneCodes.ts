...
export const flagUrl = (iso: string, width = 40) => `https://flagcdn.com/w${width}/${iso.toLowerCase()}.png`;

export const DEFAULT_PHONE_CODE_NAME = "Morocco" as const;

export const PHONE_CODE_ROWS = `
AF|Afghanistan|+93
AL|Albania|+355
DZ|Algeria|+213
...
export const PHONE_CODES = PHONE_CODE_ROWS.trim().split("\n").map((row) => {
  const [iso, name, code] = row.split("|");

  return { iso: iso.toLowerCase(), name, code, flagSrc: flagUrl(iso) };
}) as readonly PhoneCode[];

export const DEFAULT_PHONE_CODE =
  PHONE_CODES.find(({ name }) => name === DEFAULT_PHONE_CODE_NAME) ?? PHONE_CODES[0];
